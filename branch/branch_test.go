package branch

import (
	"regexp"
	"strings"
	"testing"
)

func TestSlug(t *testing.T) {
	t.Parallel()

	tests := []struct {
		input string
		want  string
	}{
		{"Add OAuth Login", "add-oauth-login"},
		{"  leading and trailing  ", "leading-and-trailing"},
		{"special!@#chars", "specialchars"},
		{"multiple   spaces", "multiple-spaces"},
		{"already-kebab", "already-kebab"},
		{"UPPERCASE", "uppercase"},
		{"feat/scope", "featscope"},
		{"", ""},
		{"!!!", ""},
	}
	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			t.Parallel()

			if got := Slug(tt.input); got != tt.want {
				t.Errorf("Slug(%q) = %q, want %q", tt.input, got, tt.want)
			}
		})
	}
}

func TestShortUUID(t *testing.T) {
	t.Parallel()

	a := shortUUID()
	b := shortUUID()

	t.Run("length is 8", func(t *testing.T) {
		if len(a) != 8 {
			t.Errorf("len = %d, want 8", len(a))
		}
	})

	t.Run("contains only lowercase hex characters", func(t *testing.T) {
		if strings.ContainsAny(a, "ABCDEFGHIJKLMNOPQRSTUVWXYZ") {
			t.Errorf("shortUUID %q contains uppercase", a)
		}
	})

	t.Run("consecutive calls return unique values", func(t *testing.T) {
		if a == b {
			t.Errorf("two consecutive calls returned the same value: %q", a)
		}
	})
}

func TestNew(t *testing.T) {
	t.Parallel()

	t.Run("well-formed input produces a four-part branch name", func(t *testing.T) {
		t.Parallel()

		b, err := New("ABC-42", "feat", "Add OAuth Login")
		if err != nil {
			t.Fatalf("New: %v", err)
		}

		parts := strings.Split(b.Name(), "@")
		if len(parts) != 4 {
			t.Fatalf("Name produced %d parts, want 4: %q", len(parts), b.Name())
		}

		t.Run("issue ID", func(t *testing.T) {
			if parts[0] != "ABC-42" {
				t.Errorf("parts[0] = %q, want %q", parts[0], "ABC-42")
			}
		})
		t.Run("branch type", func(t *testing.T) {
			if parts[1] != "feat" {
				t.Errorf("parts[1] = %q, want %q", parts[1], "feat")
			}
		})
		t.Run("slug", func(t *testing.T) {
			if parts[2] != "add-oauth-login" {
				t.Errorf("parts[2] = %q, want %q", parts[2], "add-oauth-login")
			}
		})
		t.Run("UUID is 8 lowercase hex chars", func(t *testing.T) {
			if len(parts[3]) != 8 {
				t.Errorf("parts[3] len = %d, want 8", len(parts[3]))
			}
			if matched, _ := regexp.MatchString(`^[0-9a-f]{8}$`, parts[3]); !matched {
				t.Errorf("parts[3] %q is not 8 lowercase hex chars", parts[3])
			}
		})
	})

	t.Run("all-punctuation title that produces empty slug returns error", func(t *testing.T) {
		t.Parallel()

		if _, err := New("ABC-42", "feat", "!!!"); err == nil {
			t.Error("expected error for all-punctuation title, got nil")
		}
	})

	t.Run("empty branch type returns error", func(t *testing.T) {
		t.Parallel()

		if _, err := New("ABC-42", "", "branch title"); err == nil {
			t.Error("expected error for empty type, got nil")
		}
	})
}

func TestParse(t *testing.T) {
	t.Parallel()

	b, err := Parse("ABC-42@feat@add-oauth-login@550e8400")
	if err != nil {
		t.Fatalf("Parse: %v", err)
	}

	t.Run("issue ID", func(t *testing.T) {
		if b.issueID != "ABC-42" {
			t.Errorf("issueID = %q, want %q", b.issueID, "ABC-42")
		}
	})
	t.Run("branch type", func(t *testing.T) {
		if b.btype != "feat" {
			t.Errorf("branchType = %q, want %q", b.btype, "feat")
		}
	})
	t.Run("slug", func(t *testing.T) {
		if b.title != "add-oauth-login" {
			t.Errorf("title = %q, want %q", b.title, "add-oauth-login")
		}
	})
	t.Run("UUID", func(t *testing.T) {
		if b.id != "550e8400" {
			t.Errorf("uuid = %q, want %q", b.id, "550e8400")
		}
	})
}

func TestParse_invalid(t *testing.T) {
	t.Parallel()

	cases := []struct {
		name  string
		input string
	}{
		{"empty string", ""},
		{"no @ separators", "no-separators"},
		{"two parts", "only@two"},
		{"three parts", "one@two@three"},
		{"five parts", "one@two@three@four@five"},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			t.Parallel()

			if _, err := Parse(c.input); err == nil {
				t.Errorf("Parse(%q) expected error, got nil", c.input)
			}
		})
	}
}
